...
Get Booking Data By Specific ID
    ...
    #Validations
    #Validating that api returned with success status code is 200
    Status Should Be    200
    Log    Successful getting data by posted booking id is ${id}
    #Validating that get returned with success element "firstname" in response body
    should contain    ${get_response.content}     firstname
    [RETURN]    ${get_response}
...
Update-Put Booking Data
    ...
    ${booking_dates}=    create dictionary
        ...      checkin="9/12/1010"    checkout="9/13/2010"

    ${body}=        Create Dictionary      firstname="Waheed"
     ...    lastname="Ahmed"     totalprice=9998        depositpaid="True"
     ...    bookingdates=${booking_dates}    additionalneeds="Super"

     Create Session    UpdateBookingData    ${restfulBooker_baseUrl}     headers=${header}   disable_warnings=True



    ${put_response}=    PUT On Session  UpdateBookingData   ${Booking_ENDPOINT}/${booking_Id}     json=${body}
    Status Should Be    200
